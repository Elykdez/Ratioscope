    ...
    internal class MarkdownLine
    {
        ...
        public void AddVerticalWhitespaceBefore(float spacingFontUnits)
        {
            if (!spacingFontUnits.IsPrettyCloseTo(0))
                Builder.Prepend(
                    "<size="
                        + spacingFontUnits.ToString(CultureInfo.InvariantCulture)
                        + "em>"
                        + StringHelper.ZeroWidthSpace
                        + "</size>\n"
                );
        }
        ...
        public string Finish()
        {
            if (DeleteLineAfterProcessing)
                throw new Exception(
                    "You shouldn't process this line -- it's supposed to be deleted!"
                );
