...
namespace Hypocycloid.UI
{
    internal abstract class SimpleMarkdownTag : MarkdownTag
    {
        ...
        protected override string GetMarkdownOpenTag(MarkdownRenderingSettings _) =>
            MarkdownIndicator;

        protected override string GetMarkdownCloseTag(MarkdownRenderingSettings _) =>
            MarkdownIndicator;

        protected override string GetRichTextOpenTag(MarkdownRenderingSettings _) =>
            RichTextOpenTag;

        protected override string GetRichTextCloseTag(MarkdownRenderingSettings _) =>
            RichTextCloseTag;
    }
        ...
        )
        {
            ...
            void ProcessLine(MarkdownLine line)
            {
                ...
                while (index < lineBuilder.Length - markdownOpenTag.Length)
                {
                    ...
                    int closeTagIndex = line.UnescapedIndexOf(
                        markdownCloseTag,
                        openTagIndex + markdownOpenTag.Length
                    );
                    ...
                    if (closeTagIndex > -1) // If there's a remaining tag pair in the line
                    {
                        if (openTagIndex + markdownOpenTag.Length == closeTagIndex)
                        {
                            ...
                            IsSpaceBetweenIndexesInvalidForTagging(
                                startIndex: openTagIndex + markdownOpenTag.Length,
                                endIndex: closeTagIndex - 1
                            )
                        )
                        {
                            index = openTagIndex + 1;
                        ...
                        else
                        {
                            ...
                            lineBuilder.ReplaceFirst(
                                markdownCloseTag,
                                richTextCloseTag,
                                out int richTextCloseTagStartIndex,
                                openTagIndex + richTextOpenTag.Length - 1
                            );
                ...
                bool IsSpaceBetweenIndexesInvalidForTagging(int startIndex, int endIndex)
                {
                    ...
                    if (
                        char.IsWhiteSpace(lineBuilder[startIndex])
                        || char.IsWhiteSpace(lineBuilder[endIndex])
                    )
                        return true;
