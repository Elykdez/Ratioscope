...
namespace Hypocycloid.Utils
{
    public static class CollectionExtensions
    {
        ...
        public static void RemoveAllItemsAtAndAfter<T>(this List<T> list, int startRemovingAtThisIndex)
        {
            ...
            if (startRemovingAtThisIndex >= list.Count)
                throw new ArgumentOutOfRangeException(
                    $"{nameof(startRemovingAtThisIndex)} must be less than the list's count"
                );
        ...
        public static IEnumerable<T> Subsequence<T>(this IReadOnlyList<T> list, int startIndex)
        {
            ...
            if (startIndex >= list.Count)
                throw new ArgumentException(
                    $"{nameof(startIndex)} must not be greater than the length of {nameof(list)}"
                );
        ...
        {
            return dictionary.GroupBy(p => p.Value).ToDictionary(g => g.Key, g => g.Select(pp => pp.Key).First());
        }
        ...
        {
            return dictionary.GroupBy(p => p.Value).ToDictionary(g => g.Key, g => g.Select(pp => pp.Key).ToList());
        }
        ...
        public static Tdestination[] Convert<Tsource, Tdestination>(
            ...
            Func<Tsource, Tdestination> conversionFunction
        )
        {
            if (sourceArray == null)
                return null;
        ...
        public static bool HasTheSameContentsAs_IgnoringOrder<T>(
        ...
        public static bool AllElementsHaveTheSameProperty<TSource, TProperty>(
            this IEnumerable<TSource> sourceEnumerable,
        ...
        public static bool HasTheSameKeysAndValuesAs<TKey, TValue>(
            this IReadOnlyDictionary<TKey, TValue> dicA,
            IReadOnlyDictionary<TKey, TValue> dicB,
        ...
        public static bool IsFullyIncludedWithSameKeyAndValuesIn<TKey, TValue>(
            this IReadOnlyDictionary<TKey, TValue> subsetDictionary,
            IReadOnlyDictionary<TKey, TValue> supersetDictionary,
            IEqualityComparer<TValue> valueComparer = null
        )
        {
        ...
        /// </summary>
        public static bool IsNotNullAndContains<T>(this List<T> list, T item) => list != null && list.Contains(item);
