...
namespace Hypocycloid.Utils
{
    public static class Vector3Extensions
    {
        ...
        {
            return value.x.IsBetween(minInclusive.x, maxInclusive.x)
        ...
        {
            return new Vector3(value.x.WrapRange(max.x), value.y.WrapRange(max.y), value.z.WrapRange(max.z));
        }
        ...
        public static bool IsPrettyCloseToPointingInTheSameDirectionAs(
            this Vector3 vectorA,
            Vector3 vectorB,
            float dotMarginOfError = 0.05f
        ) => Vector3.Dot(vectorA, vectorB).IsPrettyCloseTo(1, dotMarginOfError);

        public static bool IsPrettyCloseToPointingInTheOppositeDirectionAs(
            this Vector3 vectorA,
            Vector3 vectorB,
            float dotMarginOfError = 0.05f
        ) => Vector3.Dot(vectorA, vectorB).IsPrettyCloseTo(-1, dotMarginOfError);

        public static bool IsPrettyCloseToBeingPerpendicularWith(
            this Vector3 vectorA,
            Vector3 vectorB,
            float dotMarginOfError = 0.05f
        ) => Vector3.Dot(vectorA, vectorB).IsPrettyCloseTo(0, dotMarginOfError);

        public static bool IsPrettyCloseToBeingParallelWith(
            this Vector3 vectorA,
            Vector3 vectorB,
            float dotMarginOfError = 0.05f
        ) =>
            vectorA.IsPrettyCloseToPointingInTheSameDirectionAs(vectorB, dotMarginOfError)
            || vectorA.IsPrettyCloseToPointingInTheOppositeDirectionAs(vectorB, dotMarginOfError);

        public static Vector2 RemoveZComponent(this Vector3 vector3) => new Vector2(vector3.x, vector3.y);
