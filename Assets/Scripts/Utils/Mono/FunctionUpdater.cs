...
namespace Hypocycloid.Utils
{
    ...
    {
        static List<FunctionUpdater> updaterList; // Holds a reference to all active updaters
        static GameObject initGameObject; // Global game object used for initializing class, is destroyed on scene change

        GameObject gameObject;
        string functionName;
        bool active;
        Func<bool> updateFunc; // Destroy Updater if return true;
        ...
        )
        {
            ...
            GameObject gameObject = new($"<FunctionUpdater Object {functionName}>", typeof(MonoBehaviourHook));
            FunctionUpdater functionUpdater = new(gameObject, updateFunc, functionName, active);
        ...
        static void RemoveUpdater(FunctionUpdater funcUpdater)
        {
            InitIfNeeded();
